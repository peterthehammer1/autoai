...
/**
 * GET /api/analytics/ai-revenue
 * Positive metrics: revenue generated by AI, bookings, hours saved, repeat customers
 */
router.get('/ai-revenue', async (req, res, next) => {
  try {
    const cached = getCached('ai-revenue', 15 * 60 * 1000);
    if (cached) return res.json(cached);
    ...
    const [
      ...
      // AI-booked appointments with revenue (last 30 days)
      supabase.from('appointments')
        .select('quoted_total')
        .gte('scheduled_date', thirtyDaysAgo)
        .eq('created_by', 'ai_agent')
        .not('status', 'in', '("cancelled","no_show")'),
      // Total AI-handled calls (last 30 days)
      supabase.from('call_logs')
        .select('duration_seconds', { count: 'exact' })
        .gte('started_at', `${thirtyDaysAgo}T00:00:00`),
      // Repeat customers booked by AI
      supabase.from('appointments')
        .select('customer:customers!inner (id, total_visits)')
        .gte('scheduled_date', thirtyDaysAgo)
        .eq('created_by', 'ai_agent')
        .not('status', 'in', '("cancelled","no_show")')
        .gt('customers.total_visits', 0)
    ]);

    const aiAppts = aiApptsRes.data || [];
    const totalRevenue = aiAppts.reduce((sum, a) => sum + (a.quoted_total || 0), 0);
    const bookingCount = aiAppts.length;

    // Hours saved: total call count × avg 2.5 min per call (staff time saved)
    const totalCalls = aiCallCountRes.count || 0;
    const hoursSaved = Math.round((totalCalls * 2.5) / 60 * 10) / 10;

    // Unique repeat customers
    const repeatSet = new Set();
    for (const row of repeatCustomersRes.data || []) {
      if (row.customer?.id) repeatSet.add(row.customer.id);
    }
    ...
    const result = {
      ...
      repeat_customers: repeatSet.size
    };

    setCache('ai-revenue', result);
    res.json(result);
  } catch (error) {
    next(error);
  }
